...
import GridSquareSkeleton from '../Skeletons/GridSquareSkeleton'
...
const FlowDot = ({
	...
	cx: number | Animated.AnimatedInterpolation<number>
	cy: number | Animated.AnimatedInterpolation<number>
	r: number
	fill: string
	...
	return (
		<Animated.View
			...
			style={{
				...
				left: Animated.subtract(cx, r),
				top: Animated.subtract(cy, r),
				opacity,
			}}
		/>
	)
...
export default function FlowDiagram(props: flowDiagramProps) {
	...
	return (
		<View style={styles.container}>
			{
				firstLoading || isLoading ? <GridSquareSkeleton /> :

					<View style={{ position: 'relative', width: SCREEN_WIDTH, height: maxY - minY + offsetY + Math.round(5 * SCALE) }}>
						<Svg width={SCREEN_WIDTH} height={maxY - minY + offsetY + Math.round(5 * SCALE)}>
							<Defs>
							...
							<G transform={`translate(0, ${offsetY})`}>
								...
								{/* Rendered as overlay FlowDots below */}
            ...
            L ${topX + TOP_W - Math.round(12 * SCALE)} ${topY + Math.round(32 * SCALE)}`}
						...
						{/* FlowDots overlay - positioned absolutely over SVG */}
						{(() => {
							const dotStartY = topY + TOP_H + Math.round(30 * SCALE)
							const dotEndY = mainY + Math.round(3 * SCALE)
							...
							inputRange: [0, 0.02, animDuration - 0.05, animDuration],
								...
								return (
									<FlowDot
										key={`blue-flow-${i}`}
										cx={CENTER_X}
										cy={Animated.add(dotStartY + offsetY, translateY)}
										r={smallDotR}
										opacity={opacity}
										fill='white'
									/>
						...
						{(() => {
							...
							const verticalDistance = leftBoxY - lineStartY
							...
							return new Array(numDots).fill(0).map((_, i) => {
								...
								return (
									<FlowDot
										key={`left-flow-${i}`}
										cx={Animated.add(mainX, posX)}
										cy={Animated.add(lineStartY + offsetY, posY)}
										r={smallDotR}
										opacity={opacity}
										fill='white'
									/>
						...
						{(() => {
							...
							return new Array(numDots).fill(0).map((_, i) => {
								...
								return (
									<FlowDot
										key={`right-flow-${i}`}
										cx={Animated.add(mainX + MAIN_W, posX)}
										cy={Animated.add(lineStartY + offsetY, posY)}
										r={smallDotR}
										fill='white'
										opacity={opacity}
						...
						{/* Green dots */}
						{(() => {
							const dotStartY = mainY + MAIN_H
							const dotEndY = bottomY
							const totalDistance = dotEndY - dotStartY
							const numDots = 3
							const dotSpacing = 0.12
							const animDuration = 0.6
